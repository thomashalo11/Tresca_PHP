...
$query = "select * from ticket WHERE NUMERO={$ticket}";
...
if($tabella -> num_rows == 0)
    ...
    echo "<table class='table table-dark' border='2px'><tr><td>Numero</td><td>Data</td><td>Richiesta</td><td>Stato</td><td>Cliente</td></tr>";
    while($row = $tabella -> fetch_array(MYSQLI_NUM)) {
        echo "<tr><td><a href='interventi.php?matricola={$row[0]}'>{$row[0]}</a></td><td>{$row[1]}</td><td>{$row[2]}</td><td>{$row[3]}</td><td>{$row[4]}</td></tr>";
    }
    echo "</table>";
}
